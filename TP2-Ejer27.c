...
int main(int argc, char *argv[]) {
  int N, i, DNI, T, B, ContDias20, cont, sueldoM, sueldoF, sueldoTotal, Max,
      DNIMax;
  int contDias20;
  char Sexo, SexoMax;
  ...
  */
  ...
  for (i = 0; i < N; i++) {
    ...
    // B
    // busacar al sueldo mayor, el sueldo es por dias  , busco al que mas dias
    // trabaja
    if (B == 0) {
      // Valor inicial del
      Max = T;
      DNIMax = DNI;
      SexoMax = Sexo;
      B = 1; // para saber que ya esta inicialisado
    }
  ...
  printf("El empleado que mas cobra es %d , sexo %c ,sueldo %d", DNIMax,
         SexoMax, Max * 300);
